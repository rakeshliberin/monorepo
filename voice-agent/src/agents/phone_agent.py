...
class PhoneAgent(Agent):
    def __init__(self, mobile_details: GetBPFromMobileResponse):
        super().__init__(
            instructions=load_markdown("phone_agent_instructions.md").format(
                current_date=datetime.now().strftime("%d %B %Y"),
                outstanding_details=mobile_details.outstanding_details,
                billing_information=mobile_details.billing_information,
            )
        )
