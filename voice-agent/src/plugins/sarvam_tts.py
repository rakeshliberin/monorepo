...
class SarvamTTSOptions:
    ...
    target_language_code: (
        SarvamTTSLanguages | str
    )  # BCP-47 for supported Indian languages
...
class SarvamTTS(tts.TTS):
    ...
    def __init__(
        ...
        *,
        ...
        if not validate_model_speaker_compatibility(model, speaker):
            compatible_speakers = MODEL_SPEAKER_COMPATIBILITY.get(model, {}).get(
                "all", []
            )
            raise ValueError(
                f"Speaker '{speaker}' is not compatible with model '{model}'. "
    ...
    def update_options(
        self,
        *,
        target_language_code: NotGivenOr[str] = NOT_GIVEN,
